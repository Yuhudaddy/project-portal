...
function renderBars(member, memberIndex) {
  return `<div class="bar-entry-wrap"><div class="bar-entry-head"><strong>配筋資料</strong><span>依配筋圖手動輸入號數、數量及間距</span><button class="add-button compact-button" type="button" data-add-bar="${memberIndex}">＋新增鋼筋</button></div><div class="bar-entry-list">${member.bars.map((bar, barIndex) => `<div class="bar-entry"><label class="field"><span>類別</span><select data-bar-field="kind" data-member-index="${memberIndex}" data-bar-index="${barIndex}">${BAR_KINDS.map(item => `<option value="${item}" ${item === bar.kind ? "selected" : ""}>${item}</option>`).join("")}</select></label><label class="field"><span>鋼筋號數</span><input type="text" data-bar-field="size" data-member-index="${memberIndex}" data-bar-index="${barIndex}" value="${esc(bar.size)}" placeholder="例如：D25" /></label><label class="field"><span>數量／支數</span><input type="text" data-bar-field="count" data-member-index="${memberIndex}" data-bar-index="${barIndex}" value="${esc(bar.count)}" placeholder="例如：12" /></label><label class="field"><span>間距（cm）</span><input type="text" data-bar-field="spacing" data-member-index="${memberIndex}" data-bar-index="${barIndex}" value="${esc(bar.spacing)}" placeholder="例如：15" /></label><label class="field bar-note"><span>備註</span><input type="text" data-bar-field="note" data-member-index="${memberIndex}" data-bar-index="${barIndex}" value="${esc(bar.note)}" placeholder="位置／層位" /></label>${member.bars.length > 1 ? `<button class="bar-remove" type="button" data-remove-bar="${memberIndex}" data-bar-index="${barIndex}">移除</button>` : ""}</div>`).join("")}</div></div>`;
}
...
function renderCheckCard(check, index, collection, record) { const actual = record?.actual || ""; const result = record?.result || "待確認"; return `<article class="check-card ${result === "不合格" ? "is-failed" : result !== "待確認" ? "is-passed" : "is-pending"}"><div><h4>${index + 1}. ${esc(check[1])}</h4><p>${esc(check[2])}</p></div><label class="field"><span>紀錄／實測</span><input type="text" data-check-actual="${collection}" data-check-id="${esc(check[0])}" value="${esc(actual)}" placeholder="${esc(check[3] || "填寫現場結果")}" /></label><label class="field"><span>結果</span><select data-check-result="${collection}" data-check-id="${esc(check[0])}">${statusOptions(result)}</select></label></article>`; }
