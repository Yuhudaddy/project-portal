...
The site opens at `index.html`, which is the tool index. The continuous-wall entry tool is `continuous-wall.html`; its Diaphragm Wall unit record, Guide Wall review, and Rebar Cage review share one application and one PDF workflow. The template entry tool is `template.html`; it provides a mobile-first RC formwork review, measurement, pour-release, stripping, PDF, JSON, and Markdown workflow. Rebar, steel-structure, and scaffold links intentionally resolve to the custom GitHub Pages `404.html` until those tools are built. Legacy `record.html` and `checklists.html` URLs redirect to the continuous-wall tool.
