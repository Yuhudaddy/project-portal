...
def rebar_pdf():
    ...
    story = [header("鋼筋工程查驗表", "RC REBAR / FIELD REVIEW / 01–02", "3F｜柱 C1-03"), section("01｜工程概要"), meta_grid(common), Spacer(1, 3*mm), section("02｜構件資訊"), data_table(["項次", "類型", "構件編號", "軸線／位置", "強度", "寬／厚(cm)", "高度(cm)", "保護層(cm)"], [["1", "柱", "C1-03", "A-1／B-C", "SD420", "60", "80", "4"], ["2", "梁", "G3-12", "B-2／3F", "SD420", "30", "60", "4"]], [10*mm, 15*mm, 22*mm, 28*mm, 18*mm, 22*mm, 22*mm, 24*mm], small=True), Spacer(1, 3*mm), section("配筋資料"), data_table(["構件", "類別", "號數", "數量／支數", "間距(cm)", "備註"], [["柱 C1-03", "主筋", "D25", "12", "—", "四面配置"], ["柱 C1-03", "箍筋", "D13", "—", "10", "135° 彎鉤"]], [33*mm, 25*mm, 22*mm, 31*mm, 25*mm, 40*mm], small=True), PageBreak()]
    ...
    story += [header("材料與施工前查驗", "RC REBAR / FIELD REVIEW / 03", "3F｜柱 C1-03"), section("03｜材料與施工前"), data_table(["項次", "檢查項目", "判定標準", "紀錄／實測", "結果"], material_rows, [10*mm, 35*mm, 59*mm, 48*mm, 24*mm], small=True), PageBreak()]
    ...
    story += [header("鋼筋配置查驗", "RC REBAR / FIELD REVIEW / 04", "3F｜柱 C1-03"), section("04｜鋼筋配置"), data_table(["項次", "構件", "檢查項目", "判定標準", "紀錄／實測", "結果"], placement, [10*mm, 24*mm, 35*mm, 52*mm, 35*mm, 20*mm], small=True), PageBreak()]
    ...
    story += [header("接頭／保護層查驗", "RC REBAR / FIELD REVIEW / 05", "3F｜柱 C1-03"), section("05｜接頭／保護層"), data_table(["項次", "構件", "檢查項目", "判定標準", "紀錄／實測", "結果"], detail, [10*mm, 24*mm, 38*mm, 51*mm, 33*mm, 20*mm], small=True), PageBreak()]
    ...
    story += [header("鋼筋澆置前放行", "RC REBAR / FIELD REVIEW / 06", "3F｜柱 C1-03"), section("06｜澆置前放行"), data_table(["項次", "檢查項目", "判定標準", "紀錄／說明", "結果"], release, [10*mm, 40*mm, 60*mm, 42*mm, 24*mm], small=True), Spacer(1, 4*mm), section("放行判定"), meta_grid([("澆置判定", "可澆置"), ("備註", "鋼筋、接頭、保護層及介面已完成複核。"), ("複核日期", "2026/08/28")], cols=3)]
